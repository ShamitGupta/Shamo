...
// Select which campaign to generate. Defaults to the original 9709 pilot, so
// `node build_budget_batch_workflows.mjs` with no arguments reproduces
// exactly what this generator has always produced. A second campaign is
// generated by name:
//
//   node build_budget_batch_workflows.mjs igcse-0606-2024-2025
//
// See lib/campaigns.mjs for the full campaign configuration.
const campaign = resolveCampaign(process.argv.slice(2));

// Emitted as generation-time text, not a runtime branch: when a campaign has
// no year restriction (the 9709 pilot) these render to nothing extra at all,
// so the generated "Pair and Select" node is textually identical to what it
// has always been. A campaign that DOES restrict years (IGCSE 0606,
// 2024-2025 only for now) gets an extra `includedYears` filter spliced in at
// generation time instead.
const yearFilterPrelude = campaign.includedYears
  ? `const includedYears = ${JSON.stringify(campaign.includedYears)};\nconst filteredRows = rows.filter((row) => includedYears.includes(Number(row.Year)));\n`
  : "";
const manifestRowsExpression = campaign.includedYears ? "filteredRows" : "rows";
...
const prompt = [
  ...
  'Use metadata_model=' + state.config.openai_model + ' and taxonomy_version=${campaign.taxonomyVersion}.',
...
for (const question of extracted.questions || []) {
  ...
  question.metadata.taxonomy_version = '${campaign.taxonomyVersion}';
}
...
const childWorkflow = {
  name: campaign.childWorkflowName,
  nodes: childNodes,
  pinData: {},
  ...
  versionId: idFor("budget-child-workflow-version" + campaign.versionSeedSuffix),
...
const config = {
  ...
  google_document_id: '${campaign.googleDocumentId}',
  google_sheet_name: '${campaign.googleSheetName}',
  qualification: '${campaign.qualification}',
  syllabus_code: '${campaign.syllabusCode}',
...
if (config.campaign_budget_limit_usd > 15) {
  throw new Error('Safety stop: the ${campaign.sheetLabel} pilot campaign cannot exceed $15.');
}
return [{ json: config }];
`.trim();
...
${yearFilterPrelude}const normalizeDocumentType = (value) => {
...
const parsed = ${manifestRowsExpression}.map((row, rowIndex) => {
  ...
  if (String(row.Grade || '').trim() !== '${campaign.gradeLabel}') {
    throw new Error('${campaign.sheetLabel} row ' + (rowIndex + 2) + ' is not ${campaign.gradeLabel}.');
  }
...
const controllerNodes = [
  ...
  codeNode("Check Database Free-Tier Safety", checkDatabaseUsageCode, [-100, 0]),
  {
    parameters: {
      documentId: {
        __rl: true,
        value: campaign.googleDocumentId,
        mode: "list",
        cachedResultName: campaign.googleDocumentCachedName,
        cachedResultUrl:
          "https://docs.google.com/spreadsheets/d/" + campaign.googleDocumentId + "/edit",
      },
      sheetName: {
        __rl: true,
        value: campaign.googleSheetName,
        mode: "list",
        cachedResultName: campaign.googleSheetName,
      },
      options: {},
    },
...
const controllerWorkflow = {
  name: campaign.controllerWorkflowName,
  ...
  versionId: idFor("budget-controller-workflow-version" + campaign.versionSeedSuffix),
  meta: { templateCredsSetupCompleted: false },
  tags: [],
};
...
await Promise.all([
  fs.writeFile(
    path.join(outputDir, campaign.outputChildFilename),
    JSON.stringify(childWorkflow, null, 2) + "\n",
  ),
  fs.writeFile(
    path.join(outputDir, campaign.outputControllerFilename),
    JSON.stringify(controllerWorkflow, null, 2) + "\n",
  ),
  // Qualification-agnostic (see lib/campaigns.mjs), so every campaign writes
  // the same shared publish workflow under the same filename rather than a
  // byte-identical duplicate per campaign.
  fs.writeFile(
    path.join(outputDir, campaign.outputPublishFilename),
    JSON.stringify(publishWorkflow, null, 2) + "\n",
  ),
]);

console.log(`Generated guarded Shamo batch-ingestion workflows for campaign "${campaign.id}".`);
